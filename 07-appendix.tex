...
% \propsynrestloop*
% \begin{proof}
%   Let~$P$ be a predicate such that~$P(\stmt)$ is true if, and only if, all
%   execution paths within~$\stmt$ contain a matching $\ceu{\Break}$ or
%   $\ceu{\AwaitExt}$ or $\ceu{\EveryExt}$.  We shall prove the following,
%   more general, statement:
%   \begin{equation}
%     \label{prop.syn-rest.loop.eq}
%     P(\stmt)\quad\text{implies}\quad
%     \<\stmt,\ell,\nil,\theta>\nst[*]\<\stmt',\ell,e,\theta'>,
%   \end{equation}
%   where~$\stmt'=\ceu{\Break}$ or~$\isblk(\stmt',\ell)$.  The syntactic
%   restriction for loop bodies (see Section~\ref{sec.sem.restrictions}) and
%   statement~\eqref{prop.syn-rest.loop.eq} imply in
%   Proposition~\ref{prop.syn-rest.loop}.

%   We proceed by induction on the structure of~$\stmt$.  Suppose~$P(\stmt)$
%   holds.  Then the following cases are possible.
%   \begin{case}
%   \item[{[$\ceu{\Var{v\,\stmt_1}}$]}]
%     By \R{var-expd},
%     \[
%       \<\ceu{\Var{v,\stmt_1}},\ell,\nil,\theta>
%       \nst[1]\<\ceu{\AtVar{v\,\bot\,\stmt}},\ell,\nil,\theta>.
%     \]
%     The rest of the proof is similar to that of the case for $\ceu{\AtVar}$.
%     %%
%   \item[{[$\ceu{\AwaitExt(e_1)}$]}]
%   \item[{[$\ceu{\AwaitInt(e_1)}$]}]
%   \item[{[$\ceu{\EmitInt(e_1)}$]}]
%   \item[{[$\ceu{\Break}$]}]
%   \item[{[$\ceu{\IfElse{\expr}{\stmt_1}{\stmt_2}}$]}]
%   \item[{[$\ceu{\stmt_1;\stmt_2}$]}]
%     There are three subcases.
%     \begin{case}
%     \item[{[$\stmt_1=\ceu{\Nop}$]}] Then~$P(\stmt_2)$,  and by \R{seq-nop},
%       \[
%         \<\ceu{\Nop;\stmt_2},\ell,\nil,\theta>
%         \nst[1]\<\ceu{\stmt_2},\ell,\nil,\theta>.
%       \]
%       Hence, from $P(\stmt_2)$ and the induction hypothesis,
%       \[
%         \<\ceu{\stmt_2},\ell,\nil,\theta>\nst[*]\<\stmt',\ell,e,\theta'>,
%       \]
%       where~$\stmt'=\ceu{\Break}$ or~$\isblk(\stmt',\ell)$.
%     \item[{[$\stmt_1=\ceu{\Break}$]}] By \R{seq-brk},
%       \[
%         \<\ceu{\Break;\stmt_2},\ell,\nil,\theta>
%         \nst[1]\<\ceu{\Break},\ell,\nil,\theta>.
%       \]
%     \item[{[$\stmt_1\ne\ceu{\Nop,\Break}$]}] Then $P(\stmt_1)$ or
%       $P(\stmt_2)$.
%     \end{case}
%     %%
%   \item[{[$\ceu{\Loop{\stmt_1}}$]}]
%   \item[{[$\ceu{\Every{e\,\stmt_1}}$]}]
%   \item[{[$\ceu{\stmt_1\ParAnd\stmt_2}$]}]
%   \item[{[$\ceu{\stmt_1\ParOr\stmt_2}$]}]
%   \item[{[$\ceu{\Fin{\stmt_1}}$]}]
%   \item[{[$\ceu{\RunAt(\ell_1)}$]}]
%   \item[{[$\ceu{\AtVar{v\,n,\stmt_1}}$]}]
%   \item[{[$\ceu{\stmt_1\AtLoop{\stmt_2}}$]}]
%   \item[{[$\ceu{\stmt_1\AtParAnd\stmt_2}$]}]
%   \item[{[$\ceu{\stmt_1\AtParOr\stmt_2}$]}]
%   \end{case}
% \end{proof}
