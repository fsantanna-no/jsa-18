...
\fs{Acho que está bom assim como está.}

\gl{Que tal ``local'' ao invés de ``block''?  Já que ``block'' também
  significa bloquear.}
\fs{Melhor sim. Acho que ``locals'' ou ``scope'' fica mais claro.}
...
\fs{OK. Isso é muito chato mesmo, veja o que for mais fácil.}
...
\strut\fs{A definição de Env está ligeiramente diferente da implementação, mas
          imagino que você esteja ciente (se sim, ignore esse comentário).}
...
Rule~\R{par/or-expd} also appends a $\ceu{\Rest}$ to the resulting
...
So, the bootstrap reaction terminates in a memory where variable~$\ell$ is
