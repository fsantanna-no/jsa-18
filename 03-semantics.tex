...
Rule~\R{assign} transforms a~$\ceu{v\coloneqq\expr}$ into a $\ceu{\Nop}$
...
%%The~$\nst$ rules for event emission are defined as follows:
\begin{spreadlines}{\rulejot}
\begin{align*}
  \<\ceu{\EmitInt(e)},n,\nil,\theta>
...
By rule \R{emit-int}, an $\ceu{\EmitInt(e)}$ generates an internal event $e$
and becomes a $\ceu{\RunAt(n)}$, which can only resume at stack level~$n$.
This is so because rule \R{run-at} applies only if the integer~$n$ in the
$\ceu{\RunAt(n)}$ statement is equal to the stack level.  Since every~$\nst$
rule expects the input event to be empty, an application of \R{emit-int}
leads immediately to an application of \R{push} at the outer level, creating
...
%%The $\nst$ rules for conditionals are the following:
\begin{spreadlines}{\rulejot}
\begin{align*}
...
\R{seq-brk}).  Finally, a sequence whose first part ($\stmt_1$) is neither
$\ceu{\Nop}$ nor~$\ceu{\Break}$ advances to the sequence
($\stmt_1';\stmt_2)$, event ($e$), and memory ($\theta'$) resulting from
advancing just its first part (rule \R{seq-adv}).
...
statement (rule \R{loop-expd}).  The $\ceu{\Restore}$ ensures that if the
...
saved body.  Rules \R{loop-nop} and \R{loop-adv} are analogous to rules
...
loops, rules \R{loop-brk} and \R{seq-brk} would conflict.)  Rule
