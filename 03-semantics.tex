...
elaborate constructs of full \CEU can be easily defined on top of basic \CEU
primitives, as we shall discuss shortly.

Here are the statements of basic \CEU:
%%
\begingroup
\setlength{\jot}{1.5pt}
\begin{alignat*}{2}
  &\ceu{\Block{\stmt}} &\quad&\text{\it block of statements}\\
  &\ceu{\Var{v}}            &&\text{\it local variable declaration}\\
  ...
  &\ceu{\Break}             &&\text{\it break from loop}\\
  ...
  &\ceu{\CanRun(n)}         &&\text{\it can run on stack level~$n$}\\
...
\fs{Tirar de listing/figura vai perder o ref da próxima seção.}

In the above list, the metavariables~$\stmt$, $\expr$, $e$, $v$, and~$n$
denote statements, expressions, event identifiers, variable identifiers, and
nonnegative integers, respectively.  Expressions are used to denote integer
values.  They are build up from constants (0, 1, 123, \ldots) and variables
by the usual mathematical and boolean operators ($+$, $\times$, $\le$,
\ldots).

\gl{Minha ideia aqui é deixar a notação e a terminologia menos pesada.
  TODO: Dizer que expr int é suficiente; dizer que alguns do stmts são
  internos; fazer a ligação com a próxima seção (mapeamento para full \CEU).
  \textbf{Pergunta:} Como descrever o \emph{block} sem falar de ambiente?
  Estou assumindo que o usuário pode declarar variável em qualquer lugar, e
  não apenas dentro de \emph{block}, certo?}
\fs{Pelo que conversamos hoje, o \emph{Var} vai sair e só teremos o
    \emph{Block} que deve declarar todas as suas variáveis. O mapeamento entre
    full e basic fica trivial também.}
...
similar to the concrete language in Listing~\ref{lst.syntax}, there are some 
significant mismatches between \CEU and the formal semantics that require some 
clarification.

Also note that the \texttt{@}-statements do not exist in the concrete language
and are results of expansions in the transition rules to be presented in
Section~\ref{sec.sem.opsem}.
...
the value in the communication.
%
The original code in Listing~\ref{lst.await_emit.a} declares an internal event
\code{e} (ln~2) and has and \code{await} (ln~4) and \code{emit} (ln~10) to
communicate the value $1$ between the trails in parallel.
%
The translation in Listing~\ref{lst.await_emit.b} declares an additional shared
variable \code{e\_} (ln~1) to hold the emitted value (ln~9), which can be
accessed by the awaking trail (ln~5).
...
To support first-class timers, we introduce a \code{TIMER} input to notify the
passage of time, and two global variables \code{timer\_} (to hold the elapsed
time) and \code{delta\_} (to hold the residual time set to 0).

Listing~\ref{lst.TIMER.b} shows the translation from Listing~\ref{lst.TIMER.a}
for two timers in sequence (ln~1,11).
A timer first adds a (possibly) negative delta from the time it should await
(ln~1).
Then, it enters in a loop that awakes on every occurrence of \code{TIMER}
(ln~7) and decrements the time it should await (ln~8).
Each iteration of the loop checks if the timer has expired (ln~3), and sets the
new delta, which may affect a timer in sequence.
The check happens before the await because the timer may already start expired
...
The biggest mismatch between \CEU and the formal semantics is due to the
concrete \code{finalize} and the abstract \code{fin}.
%
Listing~\ref{3.lst.fin.a} shows the original program with an explicit block
(ln~1,8) that executes the statements in \code{<A>} (ln~3) immediately followed
by \code{<C>} (ln~7), and unconditionally executes \code{<B>} (ln~5) when the
block terminates or aborts.
%
The translation in Listing~\ref{3.lst.fin.b} also executes \code{<A>} (ln~1)
immediately followed by \code{<C>} (ln~5).
The difference is that the \code{fin <B>} (ln~3) in the abstract semantics
blocks the pending statement forever, which only awakes and executes when it is
aborted.
The \code{par/or} (ln~2--6) serves this purpose since it allows \code{<C>} to
