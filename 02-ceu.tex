...
\CEU is a synchronous reactive language in which programs evolve in a
sequence of discrete reactions to external events.
%
...
In \CEU, every execution path within loops must contain at least a \code{break}
or \code{await} statement to an external input
...
% %
% \fs{Não. Aí é mais por uma razão de design. O programador espera que o
% \emph{every} sempre vai responder ao evento do iterador. Se puder esperar
% outras coisas dentro, essa espectativa pode ser quebrada.}
...
in reactions to different events \code{A} and \code{B} (ln~4,7), which cannot
...
they execute in reaction to the same event \code{A} (ln~4,7).
