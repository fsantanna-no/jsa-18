...
%
\CEU supports wall-clock time with the \code{await}
...
The \code{await} statement for timers returns the current \emph{delta}, as
illustrated in Figure~\ref{lst.delta}.
Listing~\ref{lst.delta.a} is a variation of Listing~\ref{lst.timers.a} in which
the second \code{await} adds the current delta to annul it and guarantee that
\code{v} remains set to \code{1} during 1 millisecond.
...
\end{minipage}
%\rule{8.5cm}{0.37pt}
\caption{ The \code{await} statement for timers returns the current
