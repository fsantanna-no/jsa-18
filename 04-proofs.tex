...
These three properties are actually properties of transitions and are
inherited by reactions.  Recall that reactions are formally defined in terms
...
\end{align*}
In words, a basic \CEU program~$\stmt_0$ in a memory~$\theta_0$ when fed
with the external event~$e$ will react to a modified program~$\stmt_f$ and
memory~$\theta_f$ if (and only if) a finite (possibly zero) number of
transitions transforms the description~$\<\stmt_0,0,e,\theta_0>$ into
$\<\stmt_f,0,\nil,\theta_f>$.

So, from this definition, \emph{reaction} will be deterministic,
terminating, and memory bounded, if relation~$\trans[*]$ is so.
